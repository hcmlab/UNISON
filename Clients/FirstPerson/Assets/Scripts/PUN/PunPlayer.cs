...
namespace PUN
{
    public class PunPlayer : PunClient
    {
        ...
        {
            if (IsMoving && Physics.Raycast(this.transform.GetChild(0).GetChild(2).position, Vector3.down, 0.1f))
            {
        ...
        protected override void OnDestroySaveCamera()
        {
        ...
        {
            if (properties.TryGetValue("customization", out object obj))
            {
                AvatarCustomization customization = AvatarCustomization.Deserialize((object[])obj);
        ...
        private void SetupCustomization(AvatarCustomization customization)
        {
            ...
            if (customization.Body is AvatarBody.Female)
            {
                ...
                materials[5].color = customization.HairColor.GetColor();
        ...
        public void OnNewPetitionEffect(int petitionID)
        {
            ...
            {
                string petitionText = DialogUtility.CreatePetitionText(response.Data);
        ...
        public void OnRegisterPetitionVoteResult(int positiveVotes, int negativeVotes)
        {
            ...
            {
                string message = positiveVotes > negativeVotes
