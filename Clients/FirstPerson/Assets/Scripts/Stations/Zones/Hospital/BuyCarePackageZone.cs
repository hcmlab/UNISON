...
namespace Stations.Zones.Hospital
{
    public class BuyCarePackageZone : InteractableWithDialog<YesNoDialog>
    {
        ...
        private void OpenBuyCarePackageDialog()
        {
            ...
            {
                if (!HandlePossibleErrorResponse(costCarePackageResponse)) return;

                if (costCarePackageResponse.TryGetResponseAsDouble(out double costCarePackage))
                {
                    string moneyUnitsAsString = costCarePackage.ToString("N2", CultureInfo.InvariantCulture);
