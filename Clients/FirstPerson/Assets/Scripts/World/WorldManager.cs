...
namespace World
{
        ...
        IStateSubscriber<VoiceRegion>
    {
        ...
        private void OnEnable()
        {
            ...
            int gameRound = (int)RoomProperties["gameRound"];
            DayState dayState = (DayState)RoomProperties["dayState"];
            float daytimeProgress = (float)RoomProperties["daytimeProgress"];
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(ControlSettings.ControlData.ToggleMenu))
            {
                ...
                if (player != null)
                {
                    if (Cursor.visible.Equals(true))
                    {
                        player.GetComponent<PlayerManager>().DisableCursor();
                    }
                    else
                    {
                        player.GetComponent<PlayerManager>().EnableCursor();
                    }
                }
            }
        }
        ...
        {
            GameObject obj = Instantiate(masterManagerPrefab, Vector3.zero, Quaternion.identity);
            MasterManager manager = obj.GetComponent<MasterManager>();
        ...
        {
            GameObject obj = Instantiate(playerManagerPrefab, Vector3.zero, Quaternion.identity);
        ...
        {
            float daytimeDuration = GetDaytimeDuration(DayState);
        ...
        private void UpdateTriggerElements(bool force = false)
        {
            if (force || syringesEnabled != SettingsMenu.SyringesEnabled)
            {
                ...
                foreach (GameObject obj in syringeObjects)
                {
        ...
        private void UpdatePlayerPerceptibility(bool visible, bool audible)
        {
            if (otherPlayersVisible != visible)
            {
                ...
                foreach (PunPlayer player in players.Values.ToList())
                {
        ...
        public void OnUpdated(Station station, Station oldStation)
        {
            ...
            bool visible = !station.HideOtherPlayers;
        ...
        public void SubscribeToPlayers(ICollectionSubscriber<PunPlayer> subscriber, bool addInitial = false)
        {
            ...
            foreach (PunPlayer client in players.Values.ToList())
            {
        ...
        public void RegisterEvent(GameEvent gameEvent)
        {
            ...
            foreach (ICollectionSubscriber<GameEvent> subscriber in eventSubscribers)
            {
        ...
        public void SubscribeToEvents(ICollectionSubscriber<GameEvent> subscriber, bool addInitial = false)
        {
            ...
            foreach (GameEvent gameEvent in eventFeed)
            {
                subscriber.OnAdded(gameEvent);
            }
        }
        ...
        public void UnregisterMaster(PunMaster punMaster)
        {
            ...
            {
                Debug.LogError($"Tried to unregister the PUN master client {punMaster.Name} when they are not the registered PUN master client!");
                return;
            }
        ...
        public void RegisterPlayer(PunPlayer player)
        {
            ...
            {
                Debug.LogError($"Tried to register a PUN client for player {player.Name} when there already has been one registered!");
                return;
            }
            ...
            foreach (ICollectionSubscriber<PunPlayer> subscriber in playerSubscribers)
            {
        ...
        public void OnPlayerPropertiesUpdate(Player targetPlayer, Hashtable changedProps)
        {
            ...
            else if (players.TryGetValue(targetPlayer, out PunPlayer punPlayer))
            {
        ...
        {
            if (!RoomProperties.TryGetValue(key, out object obj))
            {
            ...
            float timeout = Time.time + CompareAndSwapRoomPropertyTimeout;
        ...
        public Station GetStationByType(StationType stationType)
        {
            ...
            if (stationDict.TryGetValue(stationType, out Station station))
            {
                return station;
            }
        ...
        private void OnDayStateUpdate(int gameRound, DayState dayState, float daytimeProgress)
        {
            ...
            foreach (IStateSubscriber<DayState> subscriber in dayStateSubscribers)
            {
        ...
        public (bool, string) IsInteractableActive(StationType station, ZoneType zone)
        {
            switch (zone)
            {
                ...
                case ZoneType.Buy:
                    return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("buyInteractableActive"));
                case ZoneType.Invest:
                    return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("investInteractableActive"));
                case ZoneType.SendMoney:
                    return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("sendMoneyInteractableActive"));
                case ZoneType.EarnMoney:
                    return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("earnMoneyInteractableActive"));
                case ZoneType.Learn:
                    return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("learnInteractableActive"));
                case ZoneType.Vote:
                    return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("voteInteractableActive"));

                case ZoneType.Relax:
                    return (DayState == DayState.Evening, LocalizationUtility.GetLocalizedString("relaxInteractableActive"));
                case ZoneType.Petition:
                    return (DayState == DayState.Evening, LocalizationUtility.GetLocalizedString("petitionInteractableActive"));
                ...
                case ZoneType.Entry:
                    switch (station)
                    {
                        ...
                        case StationType.TownHall:
                            return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("enterTownHallInteractableActive"));
                        case StationType.Mall:
                            return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("enterMallInteractableActive"));
                        case StationType.School:
                            return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("enterSchoolInteractableActive"));
                        case StationType.Office:
                            return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("enterOfficeInteractableActive"));

                        case StationType.Lounge:
                            return (DayState == DayState.Evening, LocalizationUtility.GetLocalizedString("enterLoungeInteractableActive"));
                ...
                case ZoneType.Exit:
                    switch (station)
                    {
                        ...
                        case StationType.TownHall:
                            return (DayState == DayState.Daylight, LocalizationUtility.GetLocalizedString("exitTownHallInteractableActive"));
        ...
        private IEnumerator UpdateTrophy()
        {
            ...
            while (isActiveAndEnabled)
            {
                yield return BackendConnection.GetPetitions(response =>
                {
                    ...
                    {
                        int numberClosedPetitions = 0;
                        foreach (Petition petition in response.Data)
                        {
