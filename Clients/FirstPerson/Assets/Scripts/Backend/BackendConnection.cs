...
namespace Backend
{
    ...
    public static class BackendConnection
    {
        ...
        {
            UnityWebRequest unityWebRequest = UnityWebRequest.Get(websiteUrl);
            yield return unityWebRequest.SendWebRequest();
        ...
        {
            (string requestUrl, UnityWebRequest unityWebRequest) = CreateWebRequest(httpRequestMethod, action, values);
            if (logAction)
            {
            ...
            bool logAction = false)
        {
            ...
            {
                List<Player> simplePlayers = JsonConvert.DeserializeObject<List<Player>>(response);
                foreach (Player simplePlayer in simplePlayers)
                {
            ...
            [NotNull] List<Scale> scaleIDs, bool logAction = false)
        {
            ...
            {
                Dictionary<string, string> deserializedResults =
                    JsonConvert.DeserializeObject<Dictionary<string, string>>(response);

                Dictionary<Scale, int> results = new Dictionary<Scale, int>();
                foreach (KeyValuePair<string, string> result in deserializedResults)
                {
