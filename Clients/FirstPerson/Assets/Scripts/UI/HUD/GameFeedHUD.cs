...
namespace UI.HUD
{
    ...
    public class GameFeedHUD : ScrollingHUD, ICollectionSubscriber<GameEvent>
    {
        ...
        {
            TMP_Text text = Instantiate(FeedObjectPrefab, FeedObjectContainer.transform).GetComponent<TMP_Text>();
        ...
        public void OnScrollDownButtonPressed() => shouldScrollToBottom = true;

        public void OnHandleClicked() => shouldScrollToBottom = false;

        private void OnScrollValueChanged(Vector2 newVal)
        {
            double newScrollPos = Math.Round(newVal.y, 2);
