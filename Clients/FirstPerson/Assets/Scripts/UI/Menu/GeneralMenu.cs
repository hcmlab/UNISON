...
namespace UI.Menu
{
    public class GeneralMenu : MonoBehaviour, ILobbyCallbacks
    {
        ...
        public void OnRoomListUpdate(List<RoomInfo> updatedRooms)
        {
            ...
            if (updatedRooms.Count <= 0) return;
            ...
            foreach (KeyValuePair<string, RoomInfo> room in roomDictionary)
            {
        ...
        private IEnumerator RefreshRejoin()
        {
            while (isActiveAndEnabled)
            {
                ...
                PlayerSaveGame playerSaveGame = LoadSave.LoadPlayerSaveGameFromFile();
                ...
                switch (response.GetResponseType())
                {
                    case ResponseType.Success:
                        if (response.TryGetResponseAsBool(out bool value))
                        {
                            ...
                            {
                                throw new UnityException("What does no success mean here? No documentation in backend.");
                            }
                        }
                        else
                        {
