...
namespace UI.Menu
{
    public class AdminMenu : MonoBehaviour
    {
        ...
        {
            StringBuilder validMasterName = new StringBuilder(masterName.Length);
            bool isMasterNameValid = masterName.Length <= gameSettings.MaxPlayerNameLength;
            foreach (char character in masterName)
            {
        ...
        {
            StringBuilder validInstanceName = new StringBuilder(instanceName.Length);
            bool isInstanceNameValid = instanceName.Length <= gameSettings.MinInstanceNameLength;
        ...
        private void ToggleButtons(bool value)
        {
            ...
            foreach (CustomButton customButton in instanceScrollView.GetComponentsInChildren<CustomButton>())
            {
        ...
        {
            List<MasterSaveGame> list = LoadSave.RefreshSaveGameData(response.Data);
            ...
            foreach (MasterSaveGame saveGame in list)
            {
                InstanceButton instanceButton = Instantiate(instanceButtonPrefab, instanceScrollView);
        ...
        {
            RoomOptions roomOptions = PunManager.DefaultRoomOptions;
