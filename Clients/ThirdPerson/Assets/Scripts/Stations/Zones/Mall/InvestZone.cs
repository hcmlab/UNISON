...
namespace Stations.Zones.Mall
{
    public class InvestZone : SpendZone<ToggleAndSpendDialog>
    {
        ...
        private List<(string, UnityAction<bool>)> GetLabelsAndOnValueChangedListeners()
        {
            ...
            for (var i = 0; i < _availableFundTexts.Count; ++i)
            {
                var fund = _availableFunds[i];
                var fundText = _availableFundTexts[i];
        ...
        private void Invest()
        {
            if (!TryGetMoneyUnits(out var moneyUnits))
            {
                return;
            }

            CurrentDialog.ToggleButtons(false, CurrentDialog.yesButton);
            var fund = _availableFunds[CurrentDialog.GetIndexOfSelection()];
            StartCooldown();
