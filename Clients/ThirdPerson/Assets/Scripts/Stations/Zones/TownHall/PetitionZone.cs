    ...
    public class PetitionZone : InteractableWithDialog<PetitionDialog>
    {
        ...
        private void AddDialogComponents(List<Petition> petitions)
        {
            ...
            foreach (var petition in petitions)
            {
                ...
                {
                    var petitionItem = Instantiate(petitionItemPrefab, CurrentDialog.GetPetitionsTransform());

                    var petitionText = DialogUtility.CreatePetitionText(petition);
                    petitionItem.summary.text = petitionText;

                    var numberPlayersAlreadyVoted = petition.YesVotes.Count + petition.NoVotes.Count;
        ...
        private void TryToOpenTemplateList()
        {
            ...
            LockPetitionZone(
                ...
                id =>
                {
                    WorldManager.TryGetPlayerByActorNumber(id, out var player);
        ...
        private IEnumerator StartOpeningTemplateList()
        {
            ...
            {
                var templateButton = Instantiate(voteItemPrefab, CurrentDialog.GetTemplatesTransform());

                var templateText = DialogUtility.CreatePetitionText(template);
        ...
        private void AskToRegisterNewPetition(Template template)
        {
            ...
            switch (template.ValueType)
            {
                ...
                case "int":
                    var possiblePetitionValue = CurrentDialog.GetPetitionValueText().Trim();
            ...
            var petitionText = DialogUtility.CreatePetitionText(template, petitionValue);
        ...
        private void RegisterNewPetition(string petitionText, int petitionTemplateID, int petitionValue)
        {
            StartCoroutine(BackendConnection.RegisterNewPetition(response =>
            {
                if (HandlePossibleErrorResponse(response))
                {
                    if (response.TryGetResponseAsInt(out var petitionID))
                    {
        ...
        public void OpenPetition()
        {
            ...
            var petitionID = currentPetitionID.Value;
