    ...
    public class RelaxZone : InteractableWithDialog<YesNoDialog>
    {
        ...
        protected override void StartAction()
        {
            StartCoroutine(BackendConnection.GetScale(response =>
            {
                ...
                {
                    if (response.TryGetResponseAsDouble(out var loungeCostMoneyUnits))
                    {
        ...
        {
            var moneyUnitsAsString = loungeCostMoneyUnits.ToString("N2", CultureInfo.InvariantCulture);
        ...
        private void Relax()
        {
            ...
            StartCoroutine(BackendConnection.Relax(response =>
            {
                ...
                if (HandlePossibleErrorResponse(response, OnActionFinished))
                {
                    if (response.TryGetResponseAsInt(out var relaxValue))
                    {
                        StartCooldown();

                        var relaxSuccessMessage = LocalizationUtility.GetLocalizedString("relaxSuccessMessage") +
