...
public static class DialogUtility
{
    ...
    {
        var scales = GetScalesWithOpenPetitions(petitions);

        var stringBuilder = new StringBuilder();
        foreach (var scaleValue in scaleValues)
        {
            var hasOpenPetition = scales.Contains(scaleValue.Key);
            if (hasOpenPetition)
            {
            ...
            switch (scaleValue.Key)
            {
                ...
                case Scale.StockIncomeTaxExempt:
                    ...
                    stringBuilder.Append(CreateTabs(Scale.StockIncomeTaxExempt))
                        .Append(IntToBoolAsString(scaleValue.Value));
                    break;
