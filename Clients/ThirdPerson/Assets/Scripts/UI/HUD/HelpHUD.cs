...
{
    /// <summary>
    ///     Shows the player the menu button and the help button. Via the help button, the player can call the master client
    ///     for help
    /// </summary>
    public class HelpHUD : MonoBehaviourHUD
    {
        ...
        private void Update()
        {
            ...
            if (hasCooldown)
            {
                var remainingCooldown = playerManager.RemainingCallForHelpCooldown;
                var percentage = remainingCooldown / playerManager.callForHelpCoolDown;
                circularProgressObject.SetPercentage(percentage);
                var rounded = Mathf.Ceil(remainingCooldown);
